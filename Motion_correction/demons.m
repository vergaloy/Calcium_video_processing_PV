...
function [D,Mp]=demons(M,F,do_display)
%% Parameters
niter           = 2^16;
...
if nlevel == 1
    ...
    for k=nlevel:-1:1
        ...
        opt = struct('niter',niter,...
            ...
            'sigma_x',sigma_x,...
            'vx',vxl, 'vy',vyl,...
            'do_display',do_display, 'do_plotenergy',0);
...
function [Mp,sx,sy,vx,vy] = register(F,M,opt)
    ...
    if ~isfield(opt,'do_plotenergy');    opt.do_plotenergy   = 0;                end;
    %% padded image
...
function showimage(varargin)
    ...
    crange    = [0 1]; % default image intensities
     varargin{1, 1}=mat2gray(varargin{1, 1});
    varargin{1, 3}=mat2gray(varargin{1, 3});
    varargin{1, 5}=mat2gray(varargin{1, 5});
    varargin{1, 7}=mat2gray(varargin{1, 7});
    for i=1:nb_args

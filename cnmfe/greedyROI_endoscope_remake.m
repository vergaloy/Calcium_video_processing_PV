function [results, center, Cn, PNR, save_avi] = greedyROI_endoscope_remake(Y, K, options,debug_on, save_avi)

% [tmp_results, tmp_center, tmp_Cn, tmp_PNR, ~] =greedyROI_endoscope_remake(v , [], options, 1, 0);
...
while searching_flag && K>0
    ...
    for mcell = 1:length(ind_localmax)
        ...
        if ind_success
            ...
            [~,~,tmp_Cn,tmp_PNR]=get_PNR_coor_greedy_PV_no_parfor(reshape(HY_box,nr2, nc2,[]));  % add by PV
